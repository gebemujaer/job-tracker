...
const SORT_OPTIONS = [
  ...
  { value: 'fit_high', label: 'Fit score high' },
  { value: 'company', label: 'Company A-Z' },
]
const STALE = 14
const EMPTY = { company: '', role: '', location: '', applied_date: new Date().toISOString().split('T')[0], follow_up_date: '', status: 'applied', fit_score_decimal: '', pay_range: '', remote_risk: '', notes: '', tags: [], job_url: '', contact: '', resume_label: '', cover_letter_label: '' }
...
function NoteModal({ app, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '1.5rem', maxWidth: 520, width: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
          <div><div style={{ fontSize: 16, fontWeight: 500 }}>{app.company}</div><div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{app.role}</div></div>
        ...
        {app.tags?.length > 0 && <div style={{ marginTop: 12, display: 'flex', gap: 6, flexWrap: 'wrap' }}>{app.tags.map(t => <span key={t} style={{ padding: '2px 8px', background: 'var(--accent-dim)', borderRadius: 20, fontSize: 11, color: 'var(--accent)' }}>{t}</span>)}</div>}
      </div>
    </div>
  )
}
...
export default function TrackerPage() {
  ...
  const openAdd = () => { setForm({ ...EMPTY, applied_date: new Date().toISOString().split('T')[0] }); setEditing(null); setShowForm(true) }
  const openEdit = (app) => {
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 3 }}>{safeApps.length} total · {safeApps.filter(a => ['applied','screening','interview'].includes(a.status)).length} active</p>
        </div>
      ...
      {stale.length > 0 && (
        <div style={{ background: 'var(--ghost-dim)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '1rem' }}>
          <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--ghost)', marginBottom: 8 }}>Likely ghosted: {stale.length}</div>
      ...
      {showForm && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: '1.5rem', marginBottom: '1.5rem' }}>
          <div style={{ fontSize: 14, fontWeight: 500, marginBottom: '1.25rem' }}>{editing ? 'Edit' : 'New application'}</div>
          <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div><label style={lStyle}>Remote work risk</label><select value={form.remote_risk} onChange={e => set('remote_risk', e.target.value)} style={sStyle}><option value="">Not set</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: '1rem', flexWrap: 'wrap' }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." style={{ flex: 1, minWidth: 150, padding: '8px 12px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 13, color: 'var(--text)', outline: 'none' }} />
        <select value={fStatus} onChange={e => setFStatus(e.target.value)} style={{ padding: '8px 12px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 13, color: 'var(--text)' }}>
          <option value="">All statuses</option>
          {Object.entries(STATUS_CONFIG).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}
        </select>
        <select value={fRisk} onChange={e => setFRisk(e.target.value)} style={{ padding: '8px 12px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 13, color: 'var(--text)' }}>
          <option value="">All risk</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option>
        </select>
        <select value={sort} onChange={e => setSort(e.target.value)} style={{ padding: '8px 12px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 13, color: 'var(--text)' }}>
          {SORT_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
      </div>

      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--text-muted)', fontSize: 14 }}>{safeApps.length === 0 ? 'No applications yet.' : 'No results.'}</div>
      ) : (
            ...
            return (
              <div key={app.id} style={{ background: 'var(--bg-card)', border: `1px solid ${isStale ? 'rgba(167,139,250,0.2)' : 'var(--border)'}`, borderRadius: 12, padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 180 }}>
                  ...
                  {app.tags?.length > 0 && <div style={{ display: 'flex', gap: 4, marginTop: 5, flexWrap: 'wrap' }}>{app.tags.map(t => <span key={t} style={{ fontSize: 10, padding: '1px 7px', background: 'var(--accent-dim)', borderRadius: 20, color: 'var(--accent)' }}>{t}</span>)}</div>}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', flexShrink: 0 }}>
                  ...
                  <button onClick={() => setNoteApp(app)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: 13, padding: '4px 6px' }}>note</button>
                  <button onClick={() => openEdit(app)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: 13, padding: '4px 6px' }}>edit</button>
                  <button onClick={() => handleDelete(app.id)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: 13, padding: '4px 6px' }}>del</button>
                </div>
              </div>
            )
          })}
